...
/* TODO: It would be nice to be compatible with monoecid signmessage. How
...
Utils.formatAmountInXmcc = function(amount) {
  return Utils.formatAmount(amount, 'xmcc', {
    minDecimals: 8,
    maxDecimals: 8,
  }) + 'xmcc';
};
...
Utils.formatUtxos = function(utxos) {
  ...
  return _.map([].concat(utxos), function(i) {
    var amount = Utils.formatAmountInXmcc(i.satoshis);
...
Utils.getAddressCoin = function(address) {
  try {
    new Bitcore_['xmcc'].Address(address);
    return 'xmcc';
  } catch (e) {
    return;
  }
