 ...
 * @param {string} opts.coin[='xmcc'] - The coin for this wallet (xmcc).
 * @param {string} opts.network[='livenet'] - The Monoeci network for this wallet.
 ...
 * @param {string} opts.coin[='xmcc'] - The expected coin for this wallet (xmcc).
 ...
 * @param {string} opts.unit - Monoeci unit used to format amounts in notifications.
 */
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['xmcc', 'bit'], value.toLowerCase());
    },
  }];
 ...
 * @param {Object} opts
 * @param {String} [opts.coin='xmcc'] (optional)
    ...
    self._getUtxosForCurrentWallet({}, function(err, utxos) {
      ...
      var info = {
       ...
       if(opts.isInstantSend===true){
        ...
        if(!validUTXO)
            return cb(new Error('InstantSend requires all Monoeci you are sending (transaction inputs) to be held for about 15-20 minutes (minimum 6 network confirmations). Please wait before using InstantSend.'));
       }
 ...
 * @param {Object} opts
 * @param {string} [opts.coin = 'xmcc'] - The coin to estimate fee levels from.
 * @param {string} [opts.network = 'livenet'] - The Monoeci network to estimate fee levels from.
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb, isInstantSend) {
  ...
  function select(utxos, cb) {
    ...
    if (totalValueInUtxos < txpAmount) {
      self.logi('Total value in all utxos (' + Utils.formatAmountInXmcc(totalValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInXmcc(txpAmount) + ')');
      return cb(Errors.INSUFFICIENT_FUNDS);
    }
    if (netValueInUtxos < txpAmount) {
      self.logi('Value after fees in all utxos (' + Utils.formatAmountInXmcc(netValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInXmcc(txpAmount) + ')');
    ...
    self.logi('Big input threshold ' + Utils.formatAmountInXmcc(bigInputThreshold));
    ...
    _.each(smallInputs, function(input, i) {
      ...
      //log.debug('The input contributes ' + Utils.formatAmountInXmcc(netInputAmount));
      ...
      //log.debug('Tx size: ' + Utils.formatSize(txpSize) + ', Tx fee: ' + Utils.formatAmountInXmcc(fee));
      ...
      if (!_.isEmpty(bigInputs)) {
        ...
        if (feeVsAmountRatio > Defaults.UTXO_SELECTION_MAX_FEE_VS_TX_AMOUNT_FACTOR) {
          ...
          //log.debug('Fee/Single-input fee: ' + Utils.formatRatio(feeVsSingleInputFeeRatio) + ' (max: ' + Utils.formatRatio(Defaults.UTXO_SELECTION_MAX_FEE_VS_SINGLE_UTXO_FEE_FACTOR) + ')' + ' loses wrt single-input tx: ' + Utils.formatAmountInXmcc((selected.length - 1) * feePerInput));
      ...
      //log.debug('Cumuled total so far: ' + Utils.formatAmountInXmcc(total) + ', Net total so far: ' + Utils.formatAmountInXmcc(netTotal));

      if (netTotal >= txpAmount) {
        var changeAmount = Math.round(total - txpAmount - fee);
        //log.debug('Tx change: ', Utils.formatAmountInXmcc(changeAmount));

        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, Bitcore_[txp.coin].Transaction.DUST_AMOUNT);
        if (changeAmount > 0 && changeAmount <= dustThreshold) {
          //log.debug('Change below dust threshold (' + Utils.formatAmountInXmcc(dustThreshold) + '). Incrementing fee to remove change.');
    ...
    if (netTotal < txpAmount) {
      //log.debug('Could not reach Txp total (' + Utils.formatAmountInXmcc(txpAmount) + '), still missing: ' + Utils.formatAmountInXmcc(txpAmount - netTotal));
  ...
  //log.debug('Selecting inputs for a ' + Utils.formatAmountInXmcc(txp.getTotalAmount()) + ' txp');
  ...
  self._getUtxosForCurrentWallet({}, function(err, utxos) {
    ...
    async.whilst(function() {
      ...
      select(candidateUtxos, function(err, selectedInputs, selectedFee) {
        ...
        //log.debug('Fee for this selection: ' + Utils.formatAmountInXmcc(fee));
      ...
      if (!err) {
        ...
        self.logi('Successfully built transaction. Total fees: ' + Utils.formatAmountInXmcc(txp.fee) + ', total change: ' + Utils.formatAmountInXmcc(change));
      } else {
 ...
 * @param {Object} opts
 * @param {string} [opts.coin = 'xmcc'] - The coin for this transaction.
 * @param {string} [opts.network = 'livenet'] - The Monoeci network for this transaction.
